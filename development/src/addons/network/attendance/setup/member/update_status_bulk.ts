...
import { RemoveAdminMember } from "../admin/member/delete";


export async function PUT_AttendanceScheduleMemberStatusBulk(dateInfos: Array<RemoveAdminMember>, statusId: number): Promise<any> {

  const _get_cookie = getUserLoginInfoCookie();
  // console.log({_get_cookie});

  const url = CONSTANTS.URLS.AKWAABA_API_BASE_URL + 'attendance/meeting-event/member/bulk-status/' + statusId,
    formData = JSON.stringify(dateInfos);

  return Swal.fire({
    title: 'Add Member Status(es)?',
