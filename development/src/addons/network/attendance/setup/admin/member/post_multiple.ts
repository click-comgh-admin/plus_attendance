...
export interface AddAdminMember {
  meetingEventId: number,
  adminId: number,
  memberId: number,
}
export async function POST_AttendanceScheduleAdminMembers(addInfos: Array<AddAdminMember>): Promise<any> {

  const _get_cookie = getUserLoginInfoCookie();
  // console.log({_get_cookie});

  const url = CONSTANTS.URLS.AKWAABA_API_BASE_URL + 'attendance/meeting-event/admin-member/bulk',
    formData = JSON.stringify(addInfos);

  return Swal.fire({
    title: 'Add Admin Member(s)?',
